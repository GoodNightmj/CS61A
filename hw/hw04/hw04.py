def shuffle(s):
    ...
    "*** YOUR CODE HERE ***"
    return [s[i // 2] if i % 2 == 0 else s[len(s) // 2 + i // 2] for i in range(len(s))]
...
def balanced(m):
    ...
    left_torque = left_mass * length(left_arm)
    right_torque = right_mass * length(right_arm)
...
def max_path_sum(t):
    ...
    if is_leaf(t):
        ...
        return root_value + max(max_path_sum(b) for b in branches(t))
